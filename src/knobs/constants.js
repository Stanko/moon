...
export const knobInputGenerator = {
  ...
  [knobTypes.RANGE]: (knob, value, handler) => {
    ...
    // Hack to add moon range input
    if (knob.moon) {
      input.classList.add('moon-range');

      const moonImagesWrapper = document.createElement('div');
      moonImagesWrapper.classList.add('moon-images-wrapper');

      const moonImagesDiv = document.createElement('div');
      moonImagesDiv.classList.add('moon-images');

      moonImagesDiv.innerHTML = moonImages.join('\n');

      moonImagesWrapper.appendChild(moonImagesDiv);
      wrapper.appendChild(moonImagesWrapper);
    }

    input.addEventListener('input', () => {
      input.setAttribute('value', input.value);
      // handler(knob.name, input.value);
    });
